...
#define SPI_BEGIN _spi->beginTransaction(SPISettings(10'000'000, MSBFIRST, SPI_MODE3))
#define SPI_END   _spi->endTransaction()
...
*/
bool PMW3360::begin(SPIClass *spi, unsigned int ss_pin, unsigned int CPI)
{
  _spi = spi;
  _ss = ss_pin;
  _inBurst = false;
  ...
  adns_write_reg(REG_Angle_Tune, constrain(ROTATIONAL_TRANSFORM_ANGLE, -127, 127));
  adns_write_reg(REG_Lift_Config, PMW33XX_LIFTOFF_DISTANCE);
...
unsigned int PMW3360::getCPI()
{
  ...
  SPI_BEGIN;
  #ifdef CONFIG_PMW3360
  cpival = adns_read_reg(REG_Config1);
  cpival = (cpival + 1) * 100;
  #endif
  #ifdef CONFIG_PMW3389
  cpival = (adns_read_reg(REG_Resolution_H) << 8) | adns_read_reg(REG_Resolution_L);
  cpival = (uint16_t)((cpival + 1) & 0xFFFF) * PMW33XX_CPI_STEP;
  #endif
  SPI_END;
...
{  
  ...
  //Serial.print("REG_Motion=0x");
  //Serial.print(adns_read_reg(REG_Motion), 16);
  //Serial.print(", ");
  ...
  //  Serial.print(burstBuffer[i], 16); Serial.print(" ");
  //}
  //Serial.println();
...
void PMW3360::adns_write_reg(byte reg_addr, byte data) {
  ...
  _spi->transfer(reg_addr | 0x80 );
  //sent data
  _spi->transfer(data);

  delayMicroseconds(35); // tSCLK-NCS for write operation
  END_COM;
  delayMicroseconds(145); // tSWW/tSWR (=120us) minus tSCLK-NCS. Could be shortened, but is looks like a safe lower bound
}
...
void PMW3360::adns_upload_firmware() {
  ...
  BEGIN_COM;
  _spi->transfer(REG_SROM_Load_Burst | 0x80); // write burst destination adress
  ...
  adns_write_reg(REG_Config2, 0x00);
  Serial.print("upload_firmware:"); Serial.println(srom_id, 16);
}
...
bool PMW3360::check_signature() {
  ...
  Serial.print("pid="); Serial.println(pid, 16);
  Serial.print("iv_pid="); Serial.println(iv_pid, 16);
  Serial.print("SROM_ver="); Serial.println(SROM_ver, 16);

  #ifdef CONFIG_PMW3360
  return (pid==0x42 && iv_pid == 0xBD && SROM_ver == 0x04); // signature for SROM 0x04
  #endif
  #ifdef CONFIG_PMW3389
  return (pid==0x47 && iv_pid == 0xB8 && SROM_ver == 0xE8); // signature for SROM 0xE8
  #endif
  return false;
}
...
void PMW3360::prepareImage()
{
  ...
  BEGIN_COM;
  _spi->transfer(REG_RawData_Burst & 0x7f);
